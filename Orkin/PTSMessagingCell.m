...
+(UIImage*)balloonImage:(BOOL)sent isSelected:(BOOL)selected {
    ...
    if (sent == YES && selected == YES) {
        ...
        return [[UIImage imageNamed:@"balloon_read_left"] stretchableImageWithLeftCapWidth:22 topCapHeight:15];
        ...
        return [[UIImage imageNamed:@"balloon_read_right"] stretchableImageWithLeftCapWidth:24
                                                                               topCapHeight:15];
    }
}
...
- (void)layoutSubviews {
    ...
    CGRect avatarImageFrame = CGRectMake(25, 0, 50,50);
    CGFloat avatarforsender = self.frame.size.width - 75.0f;
    if (self.sent == YES) {
        ballonViewFrame = CGRectMake(avatarImageFrame.origin.x + avatarImageFrame.size.width, timeLabelFrame.size.height, textSize.width + 2*textMarginHorizontal, textSize.height + 2*textMarginVertical);
        timeLabelFrame = CGRectMake(self.frame.size.width - 75.0f, ballonViewFrame.size.height , dateSize.width, dateSize.height);
        messageLabelFrame = CGRectMake(avatarImageFrame.origin.x + avatarImageFrame.size.width + textMarginHorizontal,  ballonViewFrame.origin.y + textMarginVertical, textSize.width, textSize.height);
        if(textSize.height >= avatarImageFrame.size.height)
            avatarImageFrame = CGRectMake(25.0f,textSize.height - avatarImageFrame.size.height + 2*textMarginVertical, 50.0f, 50.0f);
            
    } else {
        timeLabelFrame = CGRectMake(textMarginHorizontal, 0.0f, dateSize.width, dateSize.height);
        avatarImageFrame = CGRectMake(self.frame.size.width - 75.0f, timeLabelFrame.size.height, 50.0f, 50.0f);
        ballonViewFrame = CGRectMake(avatarforsender - textSize.width - 2*textMarginHorizontal, timeLabelFrame.size.height, textSize.width + 2*textMarginHorizontal, textSize.height + 2*textMarginVertical);
        messageLabelFrame = CGRectMake(avatarforsender  -textSize.width - 1.5*textMarginHorizontal, ballonViewFrame.origin.y + textMarginVertical, textSize.width, textSize.height);
        if(textSize.height >= avatarImageFrame.size.height)
            avatarImageFrame = CGRectMake(self.frame.size.width - 75.0f,textSize.height - avatarImageFrame.size.height + 2*textMarginVertical, 50.0f, 50.0f);
    }
